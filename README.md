...
一个直接运行真实 ASR 的端到端 Demo：客户端集成 Agora Web SDK，把麦克风音频发进 RTC 频道；当前电脑上的 Bridge 以 Agora Server SDK 入会并接收 16 kHz 单声道 PCM，再调用本机 SenseVoice。公网 VPS 只跑 Rust + Salvo 控制面，负责会话和识别文本转发，不承载音频或推理。
...
前置条件：SenseVoice 已在当前电脑的 `127.0.0.1:8094` 运行；已有 Agora App ID，以及同一频道中浏览器 UID `1001` 和 Bridge UID `9001` 的两个临时 RTC Token；VPS 子域已配置 HTTPS。
...
2. VPS 创建 `deploy/.env`，使用真实配置：
   ...
   ALLOWED_ORIGIN=https://asr.example.com
...
4. 当前电脑启动真实 Bridge：
   ...
   当前 Mac 的 `.venv` 已用 `/Users/dev/miniconda3/bin/python3.13` 创建并安装完整依赖，无需重装。`start-real.sh` 会先检查配置和 SenseVoice 健康状态，再启动真实 Bridge。

5. 打开 HTTPS 页面，点击“开始识别”，授权麦克风并讲话。页面显示的文字来自本机 SenseVoice 实际推理。

在启动 Agora 前，可单独确认当前 SenseVoice HTTP 链路：
...
本仓库已针对当前 SenseVoice 服务的 JSON + base64 协议实现并实际验证，不需要再改适配器。完整验收清单见 [`docs/REAL_DEMO_CHECKLIST.md`](docs/REAL_DEMO_CHECKLIST.md)。
...
这些限制只影响并发、凭据管理和可靠性，不会把音频或 ASR 替换成模拟数据；演示主流程中的 Agora 音频和 SenseVoice 推理都是真实的。生产化下一步是接入官方 AccessToken2 builder 动态签发短期 Token，然后加入鉴权、限流、多会话路由、指标和断线恢复。

仓库仍保留 `mock-bridge`，仅供开发者在 Agora 或 SenseVoice 故障时定位控制面问题。它不是部署、验收或给老板演示的前置步骤。
